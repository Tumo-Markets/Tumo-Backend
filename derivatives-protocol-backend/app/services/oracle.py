...
class PythOracleService:
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    logger.error(f"Failed to fetch price for {price_feed_id}: {response.status}")
                    return None
    ...
    async def get_latest_prices(self, price_feed_ids: List[str]) -> Dict[str, PriceData]:
        """
        ...
        try:
            ...
            # Build query params
            url = f"{self.http_endpoint}/api/latest_price_feeds"
            params = [(f"ids[]", feed_id) for feed_id in price_feed_ids]
    ...
    async def get_price_update_data(self, price_feed_id: str) -> Optional[bytes]:
        """
        Get price update data (VAA) for on-chain verification.
        
        This data should be sent with transactions to update prices on-chain.
        
        Args:
            price_feed_id: Pyth price feed ID
            
        Returns:
            Price update data as bytes or None
        """
        try:
            session = await self._get_session()
            ...
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    logger.error(f"Failed to fetch VAA for {price_feed_id}: {response.status}")
                    return None
                ...
                # VAA is hex-encoded
                vaa_hex = data[0]
                return bytes.fromhex(vaa_hex[2:] if vaa_hex.startswith("0x") else vaa_hex)
    ...
    def is_price_confident(
        self, 
        price_data: PriceData, 
        max_confidence_ratio: Decimal = Decimal("0.01")
    ) -> bool:
        """
